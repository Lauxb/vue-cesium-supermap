...
div
  md-whiteframe(md-tag="md-toolbar").top
    .md-toolbar-container
      ...
      md-button.md-icon-button(href="https://github.com/lauxb/vue-cesium-supermap")
        md-icon(md-iconset="iconfont icon-github")
  md-sidenav.md-left.md-fixed.main-nav(ref="sidenav")
    md-toolbar(md-theme="white").logo
      router-link.link(:to="`/`")
        img(src="//lauxb.github.io/vue-cesium-supermap/favicon.png")
  ...
  mounted () {
    this.$router.afterEach(route => {
      this.$nextTick(() => {
        ...
        $table.forEach($t => {
          $t.outerHTML = `<div class="doc-table md-table md-theme-default md-whiteframe md-whiteframe-1dp">${
            $t.outerHTML
          }</div>`
        })
      })
