  ...
  watch: {
    ...
    infoBox (val) {
      ...
      if (
        ...
        let infoBoxContainer = getDocumentByClassName(
          viewerContainer.children,
    ...
    geocoder (val) {
      ...
      let toolbar = getDocumentByClassName(
        viewer.container.firstChild.children,
      ...
      if (
        ...
        let geocoderContainer = getDocumentByClassName(
          toolbar.children,
        ...
        let geocoder = new Cesium.Geocoder({
          ...
          geocoderServices: Cesium.defined(this.geocoder)
            ? Cesium.isArray(this.geocoder) ? this.geocoder : [this.geocoder]
            : undefined,
          scene: viewer.scene,
          viewer: viewer
        })
    ...
    sceneModePicker (val) {
      ...
      if (
        ...
        if (!this.scene3DOnly && this.sceneModePicker === true) {
          let sceneModePicker = new Cesium.SceneModePicker(
            toolbar,
            viewer.scene
          )
    ...
    projectionPicker (val) {
      ...
      if (
        ...
        let projectionPicker = new Cesium.ProjectionPicker(
          toolbar,
          viewer.scene
        )
...
Either specify options.terrainProvider instead or set options.baseLayerPicker to true.`)
        ...
        if (createBaseLayerPicker) {
          ...
          let imageryProviderViewModels = Cesium.defaultValue(
            ...
            createDefaultImageryProviderViewModels(Cesium, cesiumPath)
          )
          let terrainProviderViewModels = Cesium.defaultValue(
            this.terrainProviderViewModels,
            createDefaultTerrainProviderViewModels(Cesium, cesiumPath)
          )
    ...
    animation (val) {
      ...
      if (
        ...
        let animationContainer = getDocumentByClassName(
          viewerContainer.children,
        ...
        animation.viewModel.timeFormatter = function (date, viewModel) {
          var dateZone8 = Cesium.JulianDate.addMinutes(
            date,
            hour,
    ...
    timeline (val) {
      ...
      if (
        ...
        let timelineContainer = getDocumentByClassName(
          viewerContainer.children,
    ...
    fullscreenButton (val) {
      ...
      if (
        ...
        let fullscreenContainer = getDocumentByClassName(
          viewerContainer.children,
        ...
        let fullscreenButton = new Cesium.FullscreenButton(
          fullscreenContainer,
          document.body
        )
    ...
    'viewer.fullscreenButton.viewModel.isFullscreenEnabled' (val) {
      ...
      if (viewer.fullscreenButton) {
        viewer.fullscreenButton.container.style.display = val
          ? 'block'
          : 'none'
        let right = 0
        if (Cesium.defined(viewer.vrButton)) {
          viewer.vrButton.container.style.right =
            viewer.fullscreenButton.container.clientWidth + 'px'
          right += viewer.fullscreenButton.container.clientWidth
        }
        if (Cesium.defined(viewer.timeline)) {
          viewer.timeline.container.style.right =
            right + viewer.fullscreenButton.container.clientWidth + 'px'
    ...
    vrButton (val) {
      ...
      if (
        ...
        let vrContainer = getDocumentByClassName(
          viewerContainer.children,
        ...
        let vrButton = new Cesium.VRButton(
          vrContainer,
          viewer.scene,
          document.body
        )
    ...
    'viewer.vrButton.viewModel.isVREnabled' (val) {
      ...
      if (viewer.vrButton) {
        ...
        if (Cesium.defined(viewer.timeline)) {
          viewer.timeline.container.style.right =
            right + viewer.vrButton.container.clientWidth + 'px'
          viewer.timeline.resize()
        }
      }
    },
    navigation (val) {
      ...
      if (Cesium.defined(viewer.navigation) && !val) {
        ...
        let navigationContainer = getDocumentByClassName(
          viewerContainer.children,
        ...
        let navigation = new Cesium.Navigation({
  ...
  methods: {
    ...
    resizeControl () {
      ...
      if (
        animationExists &&
        window.getComputedStyle(viewer._animation.container).visibility !==
          'hidden'
      ) {
      ...
      if (
        timelineExists &&
        window.getComputedStyle(viewer._timeline.container).visibility !==
          'hidden'
      ) {
      ...
      if (Cesium.defined(viewer.navigation)) {
        let toolbarContainer = getDocumentByClassName(
          viewer.container.firstChild.children,
          'cesium-viewer-toolbar'
        )
        let navigationContainer = getDocumentByClassName(
        ...
        if (Cesium.defined(viewer.infoBox)) {
          viewer.infoBox.container.firstChild.style.right =
            navigationContainer.clientWidth + 'px'
        }
      }
    ...
    init (Cesium) {
      ...
      const viewer = new Cesium.Viewer($el, {
        ...
        navigationInstructionsInitiallyVisible: this
          .navigationInstructionsInitiallyVisible,
        ...
        automaticallyTrackDataSourceClocks: this
          .automaticallyTrackDataSourceClocks,
      ...
      if (Cesium.defined(this.camera)) {
        viewer.camera.setView({
          destination: Cesium.Cartesian3.fromDegrees(
            this.camera.position.longitude,
            this.camera.position.latitude,
